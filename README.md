...
- 같은 모드에서 저속 복셀 지도와 플레이어 이동 경로도 함께 누적한다. HUD는
  `가림 CPU · 삼각형 N · 복셀 N`을 표시하고, 운영자 뷰에서는 복셀 공간·Ninja 위치·anchor
  상태·플레이어 위치와 경로를 확인한다. 운영자 화면을 닫아도 AR render loop, anchor 갱신,
  depth 읽기와 지도 누적은 계속된다.
- ⚠️ 코드·자동 테스트는 완료했으며 갤럭시 S26 Ultra에서 손과 물체 가림 품질은 실기기 검증이 필요하다.

### 4-5. 3D 공간 복원 — 포인트클라우드 (`?depth=cloud` 진단 모드)
...
### 4-6. 복셀 재구성 + 운영자(서버) 3D 뷰 (`?occlusion=cpu`, `?depth=cloud`)
...
- `운영자 뷰` 버튼을 누르면 같은 화면에서 별도 캔버스로 전환되어, **오빗 카메라
  ...
  으로 복귀.
- HUD와 운영자 상태의 `복셀 N`은 원시 depth 샘플 수가 아니라 3회 이상 관측되어 solid가 된
  실제 복셀 수다. 운영자 상태에는 anchor/fallback, Ninja·플레이어 좌표, 경로 점 수도 표시한다.
...
`?depth=cloud&occlusion=cpu`처럼 두 값을 동시에 넣으면 통합 CPU 게임 모드를 우선한다. 오클루전과
지도는 depth를 각각 다시 읽지 않고 같은 frame snapshot을 공유한다.
